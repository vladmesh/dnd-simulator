...
- Войти в локацию БЕЗ своей encounter-таблицы, лежащую в регионе с таблицей → промотать время / двигаться, чтобы прокрутить ролл активации
...
- В локации с ночным тегом встречи (`time_of_day: night`) промотать время в ночь (Master → Advance time), затем сравнить с днём
...
## 16. Control Interfaces & Identity (Sprint 020)

### 16.1 Role selector + три линзы мастер-панели
- На лендинге выбрать роль в селекторе, открыть `/master` для каждой: Создатель миров / Мастер / Админ
- **Ожидание:** worldbuilder — только вкладка «Миры» (свои миры, без сессий); DM — «Миры» + «Сессии» (сессии скоупятся по `created_by`); admin — все миры без Fork/Delete + кросс-сессионный park-view, `SessionView` помечен «Наблюдение (только чтение)» (только Мир/Существа/Лента, без Время/Сохранения); player/пусто — полный god-mode (все вкладки + «Изменить»)

### 16.2 Spectator live observe feed
- В одной вкладке — игрок в сессии генерирует события (атака → бой); в другой как DM/admin открыть `/master/{sid}` → вкладка «Лента»
- **Ожидание:** лента read-only стримит события боя с бейджами типа (`combat_started`/`entity_attack`/`entity_move`/`round_start`) и RU-описаниями; до событий — «Ожидание событий…»; подключение/отключение зрителя НЕ выселяет сессию, пока подключён игрок

### 16.3 Идентичность в WS + faction display-name
- Войти игроком, проверить WS URL и строку репутации после убийства NPC с фракцией
- **Ожидание:** WS несёт `user_id`/`role` из селектора; строка репутации показывает display-name фракции («Силы Королевства»), а не сырой слаг (`kingdom`)

### 16.4 Персистентность XP через save/reload
- Набрать XP (убить `xp_dummy` в `level_up_test`), отложить level-up (закрыть модалку), Сохранить через master Saves → Загрузить → переоткрыть сессию
- **Ожидание:** `experience`/`level_up_available` переживают round-trip (модалка level-up снова авто-открывается); `POST /level-up` после reload проходит (200, не 400), уровень L1→L2, HP растёт
