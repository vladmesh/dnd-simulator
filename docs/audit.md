...
|-----------|-------|----------|
| `adapters/api/app.py:85-92` | CORS origins now configurable via `CORS_ALLOWED_ORIGINS` + credentials disabled when `*` (fixed d459e19); `allow_methods`/`allow_headers` still `["*"]` | low |
