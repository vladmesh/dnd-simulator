...
`World.handle_event` теперь обрабатывает очередь событий, возвращённых слоями через `ActionResult.events`,
доставляет каждый каскад всем слоям кроме породившего и возвращает полный каскад вызывающему коду. Тест ecology больше не воспроизводит отсутствующий production-шаг
