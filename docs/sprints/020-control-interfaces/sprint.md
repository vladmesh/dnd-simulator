...
**Current:** Phase 4 (Save robustness & i18n polish) COMPLETE — 2026-06-29. Phases 1-3 COMPLETE. All sprint phases done — ready for audit. Phase 4 (3 tasks): persist XP/level_up across save/load (closes `player-xp-not-persisted`) → combat/event-log i18n + faction-name leak fix (closes the phase-3 `kingdom` finding) → handler error-string i18n sweep (closes `combat-log-i18n-gaps`). Integration 166 passed (stable across 3 clean runs; two earlier failures of `test_player_state_xp` traced to a stale docker build cache, not code — clean bisects at phase-3-close and task-1 both passed). E2E green ([phase4-report](e2e/phase4-report.md)) — RU combat log + faction display name («Силы Королевства», not `kingdom` slug) + RU handler errors verified end-to-end; 1 quick fix applied (frontend `EventLog.tsx` round/move strings — remaining client-side half of `combat-log-i18n-gaps`), 0 blockers. `make check` green (backend 2317, frontend 260).

Phase 3 (Spectator-listener + disconnect-debounce) COMPLETE — 2026-06-29. Phase 1 + 2 COMPLETE. Phase 3 (4 tasks): spectator-listener primitive → disconnect grace-period (closes `session-disconnect-debounce`) → `?spectate=true` WS endpoint → frontend Live observe feed. Integration 166 passed (5 spectator-WS tests + 1 grace-period test added during implementation); E2E green ([phase3-report](e2e/phase3-report.md)) — Live feed verified end-to-end across two tabs (player generates combat events, admin+DM spectator streams them read-only), spectator churn never evicts session, 0 blockers. One minor i18n leak (`kingdom` faction id in RU reputation log) noted for Phase 4 (`combat-log-i18n-gaps`). Ready for Phase 4 task generation.
