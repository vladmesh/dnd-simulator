...
Sprint 017 закрыт за один календарный день (все 5 фаз). Введена полная система опыта и уровней: XP-by-CR, PHB thresholds, грант XP при kill (omniscient), `level_up_available` флаг, `perform_level_up` с классовыми deltas. Исправлен PHB-баг sprint 015: Paladin теперь получает Fighting Style / Divine Smite / spell slots только на L2. Добавлен Fighter L2 Action Surge (extra action / short rest) и Rogue L2 HP bump. Frontend — класс-условная `LevelUpModal` (Paladin выбирает Fighting Style, Fighter/Rogue confirm-only), dedicated `level_up_arena` world для E2E. Phase 4 погасила 6 багов из phase-3 E2E (per-location battle_map override, fail-fast coord validator, single canonical player для combat sidebar, Cancel = defer). Phase 5 (post-audit cleanup) — переезд level-up+status логики в `GameService` с типизированным `PlayerStatusData` DTO, unit-тесты для `rules/leveling` и `perform_level_up`, schemas `Any → object`, и главное — fix non-determinism в `roll_initiative` / `BattleMap.place_randomly` (использовали свежий `random.Random()`, обходили `DND_DICE_SEED`; переведены на `get_global_rng()`).

**Ключевые метрики:** 142 integration + 2174 unit тестов зелёные. 5 phase-5 tasks + Phase 4 bug sweep + sprint 015 bug fix в одном спринте. Ноль blocker'ов в post-audit E2E.

**Deferred:** `mutable-turn-budget`, `mutable-resource-pool`, `test-gap-ws-*` (disconnect, reaction prompts, concurrent messages) — уехали в BACKLOG.md (секция "From audit 2026-04-13").
