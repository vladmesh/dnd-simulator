...
InnerSelf: типизированные отношения, настроение, цели и накопление alignment плюс журнал, мысли и текущий разговор. Дельта-лог для LlmBrain (~15 строк). MemorySummarizer сжимает журнал через LLM.

### Phase 2.5a — Wiring суммаризатора
Триггеры суммаризации: конец боя, конец разговора, переполнение журнала. Суммаризация с точки зрения каждого NPC.
