...
Активные носители InnerSelf копят события через фильтр восприятия. Одна точка переваривания срабатывает на конце боя, active → dormant, завершении/прерывании намерения и переполнении буфера; тело пока обновляет дневник NPC через MemorySummarizer.
