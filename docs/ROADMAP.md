...
→ [план спринта](sprints/015-paladin-smite/sprint.md)
...
→ [план спринта](sprints/018-lairs-encounters-loot/sprint.md)
...
→ [план спринта](sprints/019-control-plane-prep/sprint.md)

### Sprint 020 — Control Interfaces (фазы 1-4)
Проекция единого control-ядра на три человеческие роли (worldbuilder / DM / админка) через минимальную identity-модель и spectator-listener; попутно закрыт кластер session/save/i18n-багов. Phase 1: identity keystone — `service/identity.py` (`Role` StrEnum, `Identity`, `resolve_identity`) + `get_identity` request-seam (заголовки `X-User-Id`/`X-Role`, невалидная роль → 400, без login/паролей/БД), `creator`-атрибуция на мирах + `created_by` на сессиях, фронт-селектор роли. Phase 2: three-lens projection `/api/master/*` — **projection-only** (линза скейпит UI, бэкенд остаётся открытым god-mode, без 403): worldbuilder (свои миры, без вкладки сессий), DM (свои миры + свои сессии по `created_by` + hot-controls), админка (read-only кросс-сессионный park-view, observe-only `SessionView`), player/null fallback (полный god-mode); закрыт остаток `master-panel-creature-inventory`. Phase 3: spectator-listener в `GameSession` (`add_spectator`/`remove_spectator`, read-only broadcast, «пусто» keyed на player-листенерах) + disconnect grace-period (отложенный `threading.Timer` evict, reconnect в окне отменяет — закрывает `session-disconnect-debounce`) + `?spectate=true` WS endpoint + фронт Live observe feed для DM/админки. Phase 4: `player-xp-not-persisted` (XP/`level_up_available` round-trip через `to_full_save_data` + `PlayerContent` + restore в `load_state`) + `combat-log-i18n-gaps` (недостающие msgid'ы loot/lay-on-hands/action-surge/conditions + утечка faction-id чинится `faction_name` через `QueryType.FACTION_NAME` + свип ~20 handler-ошибок в `_()`).
→ [план спринта](sprints/020-control-interfaces/sprint.md)
