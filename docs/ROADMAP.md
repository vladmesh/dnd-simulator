...
InnerSelf: типизированные отношения, настроение, цели и накопление alignment плюс журнал, мысли, текущий разговор и персистентный структурный буфер воспринятых событий. Дельта-лог для LlmBrain (~15 строк). MemorySummarizer сжимает журнал через LLM.

### Phase 2.5a — Wiring суммаризатора
Активные носители InnerSelf копят события через фильтр восприятия. Одна точка переваривания срабатывает на конце боя, active → dormant, завершении/прерывании намерения и переполнении буфера. Перед дневником она всегда применяет чистую правиловую дельту: атака на носителя → `hates` + `angry`, смерть союзника → `grieving`, смерть цели → `kill achieved`/`protect failed`; собственная атака на союзника копит chaos/evil evidence. Alignment пока не сдвигается, только копится; журнал NPC поверх дельты обновляет MemorySummarizer.
