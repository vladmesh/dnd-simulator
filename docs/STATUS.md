...
**Position:** Control-plane спроецирован на три человеческие роли — Sprint 020 (`control-interfaces`) закрыт: identity/role keystone (`service/identity.py`, projection-only), three-lens master-проекция (worldbuilder/DM/admin + player god-mode), spectator-listener + disconnect grace-period + Live observe feed, кластер session/save/i18n-багов закрыт (`player-xp-not-persisted`, `combat-log-i18n-gaps`, `session-disconnect-debounce`, `master-panel-creature-inventory`). По [ROADMAP](ROADMAP.md) дальше: Level 2 (расходуемые ресурсы), Level 3 (заклинания, интерактивные объекты), автономные тики NPC; мультиплеер и полная auth — отдельные тяжёлые спринты.
**Next:** Активного спринта нет. Топ бэклога: `quest-system` (**must** — цели/триггеры завершения/награды, минимум fetch/kill/escort) — главный продуктовый кандидат. Дальше: `key-npcs`, `loot-drops-monsters`. См. [BACKLOG](BACKLOG.md) / [ROADMAP](ROADMAP.md).
**Blockers:** нет.
...
|--------|------|---------|-----------|
| 020-control-interfaces | Проекция control-ядра на три роли (worldbuilder/DM/admin) через identity-keystone (projection-only) + spectator-listener + Live observe feed; кластер session/save/i18n-багов (`player-xp-not-persisted`, `combat-log-i18n-gaps`, `session-disconnect-debounce`, `master-panel-creature-inventory`) | 2026-06-29 | 2026-06-29 |
