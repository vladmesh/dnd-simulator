...
**Started:** 2026-06-28
**Phase:** 3 — Visible gaps + backlog reconcile + dead code (all 3 tasks done; ready to close phase) — 2026-06-29
...
Phase 3 — все 3 таски done: (1) combat-log i18n + encounter-spawned перцептор (commit d3a2193); (2) фронт-гейт Attack/Talk на трупах в `Perception.tsx` (commit 17d2fb6); (3) удаление 4 мёртвых функций (`refund`/`walk_path`/`prone_stand_cost`/`to_save_data`) + сверка бэклога (8 айтемов помечены). Task 3 вскрыл латентный баг `player-xp-not-persisted` (XP/level_up не переживают современный save/reload — `to_full_save_data` не сериализует `experience`; `to_save_data`+`load_save_data` round-trip жил только в backward-compat ветке) — занесён в BACKLOG как **should**, фикс вне гранулы. Integration/E2E ещё не гоняли по фазе 3 — ready to close phase.
