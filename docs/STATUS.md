...
вернуть короткую мысль, а prompt получает явные секции личности; чужая речь в буфере и prompt помечена
как услышанная. Переваривание принимает один JSON fence и быстро возвращается к rules fallback при
ограниченном timeout/retry.
