...
**Цель:** оружие как данные (YAML) + timed conditions для Bless.

**Что сделано:**
1. ✅ `WeaponDef` в `core/items.py` — frozen dataclass (attack_name, damage, reach, ability, modifier, is_magic, is_finesse, grant_conditions, grant_actions).
2. ✅ `ItemType.WEAPON` + `Item.weapon_def: WeaponDef | None`.
3. ✅ `Creature.equipped_weapon: Item | None` — предэкипировано из YAML, отдельно от inventory.
4. ✅ `get_weapon_attack()` в `rules/weapons.py` — строит Attack из equipped weapon или fallback на creature.attacks / unarmed. Finesse: `max(STR, DEX)`.
5. ✅ `WeaponActionProvider` в `rules/action_provider.py` — третий провайдер, читает `grant_actions` из WeaponDef.
6. ✅ Timed conditions: `creature.conditions: dict[Condition, int | None]` (ConditionsMap). `tick_conditions()` в `rules/conditions.py` декрементирует таймеры.
7. ✅ `ActionType.BLESS` + `handle_bless()` — bonus action, даёт `Condition.BLESSED` на 3 раунда. `resolve_attack()` роллит +d4 при BLESSED.
8. ✅ 4 оружия в arena.yaml (палка, меч, рапира finesse, благословенный меч с BLESS).
9. ✅ Content loader: `parse_items()`, `_parse_weapon_def()`, `parse_equipped_weapon()`.
10. ✅ Save/load, perception, LLM tools, frontend — всё подключено.

**Отклонения от плана:**
- `WeaponDef.ability` — nullable (`Ability | None`), defaults to STR при отсутствии в YAML. План предлагал `Ability = Ability.STR` как default.
- `grant_conditions` парсится, но equip/unequip actions не реализованы (PoC — оружие предэкипировано). Пассивные conditions от оружия не применяются автоматически.
- `get_weapon_modifier()` — отдельная функция для magic bonus, не часть `get_weapon_attack()`.
...
- **Modifier pipeline / derived stats** ✅ реализован (`rules/modifiers.py`). Handler-ы используют `effective_ac()` через combat_manager.
