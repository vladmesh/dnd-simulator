...
- [ ] **could** `combat-log-target-label-case` — в RU боевом логе строки атаки/смерти подставляют воспринимаемую расу в именительном падеже в слот, требующий винительного: «Ты атакуешь **человек**» (надо «человека»), «**человек** погибает». При этом строки инициативы/перемещения/лута/«получаешь N опыта» используют настоящее имя (Марта/Лира/XP-манекен) — рассинхрон имя-vs-раса в одном бою. Преэкзистинг (построение perceive-лейбла в `perception.py`); i18n Sprint 020 перевёл шаблон, но не падеж/выбор лейбла интерполянта. Замечено E2E post-audit sprint 020. Фикс: единый выбор лейбла цели (имя при known, иначе раса) + падежные формы для подстановки
- [ ] **could** `combat-log-content-names-en` — в RU боевом логе названия атак оружия (`longsword slash`) и источник модификатора (`+2 str`) остаются английскими внутри переведённой строки урона («4 урона (1d8 рубящий + +2 str)»). Это контент-имена из YAML-каталогов и сокращения характеристик, не gettext-msgid, поэтому вне scope i18n Sprint 020 (плейсхолдеры `{weapon}`/`{roll}` НЕ текут — значения интерполируются верно). Локализация контент-имён — отдельная задача (перевод каталогов оружия + маппинг сокращений СИЛ/ЛОВ/…). Замечено E2E post-audit sprint 020
- [ ] **could** `rep-log-faction-raw-id-fallback` — строка репутации показывает сырой `faction_id` («Твоя репутация с **monsters** изменилась»), когда в мире нет политического определения фракции с display-name (минимальная арена `level_up_test`). Механика by-design (Sprint 020 phase 4: `faction_name` через `QueryType.FACTION_NAME`, при неразрешимости → фолбэк на `faction_id`), хедлайн-кейс `sword_vale` → «Силы Королевства» работает. Косметически сырой id в RU-логе нежелателен: либо дружелюбный фолбэк («неизвестная фракция»), либо требовать display-name у фракций. Замечено E2E post-audit sprint 020
- [ ] **could** `ws-reconnect-listener-error-noise` — при reconnect игрока (React StrictMode двумаунт в dev) бэкенд логирует один `listener_error` уровня ERROR на `WsEventListener.on_turn`: открываются две WS-связи, одна пишет `on_turn` в сокет, который cleanup StrictMode уже закрывает → исключение, этот листенер снимается; выживший листенер работает, сессия продолжается (grace-period из `session-disconnect-debounce` держит счётчики корректными). Только dev (прод-сборка не двумаунтит эффекты), функционального влияния нет, но шумит ERROR-логом. Замечено E2E post-audit sprint 020. Низкий приоритет — подавить/понизить уровень для ожидаемого «socket closing» на reconnect
