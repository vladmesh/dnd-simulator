...
def handle_use_item(actor: Creature, action: Action, emit_fn: EmitFn, ctx: ActionContext, world: World) -> ActionResult:
    ...
    return ActionResult(
        success=False,
        error=_("Cannot use item of type '{item_type}', try equipping it instead").format(
            item_type=item.item_type.value
        ),
    )
...
def handle_lay_on_hands(
    ...
    if pool.current_uses < amount:
        return ActionResult(
            success=False,
            error=_("Insufficient pool: {remaining} remaining, need {amount}").format(
                remaining=pool.current_uses, amount=amount
            ),
        )
    ...
    if target_id is not None:
        ...
        if target_entity is None or not isinstance(target_entity, CreatureType):
            return ActionResult(success=False, error=_("Target '{target_id}' not found").format(target_id=target_id))
