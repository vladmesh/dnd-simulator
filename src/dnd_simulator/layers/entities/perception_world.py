...
def combat_started(event: Event, observer: Creature, get_entity: GetEntityFn) -> str:
...
def round_start(event: Event, observer: Creature, get_entity: GetEntityFn) -> str:
    payload = event.payload
    assert isinstance(payload, RoundStartPayload)
    return _("— Round {n} —").format(n=payload.round_number)


def squad_move(event: Event, observer: Creature, get_entity: GetEntityFn) -> str:
    payload = event.payload
...
def squad_combat(event: Event, observer: Creature, get_entity: GetEntityFn) -> str:
    payload = event.payload
