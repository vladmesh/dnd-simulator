...
name: go
description: >
  Determine the next step in the sprint pipeline and execute it. Reads STATUS.md, analyzes current
...
Read `docs/sprints/` — find the highest-numbered sprint folder. Read its `STATUS.md` and `sprint.md`. Also check:
...
#### No sprint folder or STATUS says COMPLETE
...
#### Fresh audit exists, STATUS.md has NO "Audit Triage" section
→ **`/audit_triage`** — audit needs to be triaged.

#### Audit triaged, STATUS.md shows sprint-relevant items went to a refactor phase, that phase has no tasks
...
#### Audit triaged (marker in STATUS.md), no E2E report dated AFTER the audit
...
If the state is ambiguous (e.g. STATUS.md is stale or contradictory) — describe what you see and ask the user instead of guessing.
