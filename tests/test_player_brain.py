...
class TestPlayerBrainQueue:
    def test_submit_unblocks_choose(self) -> None:
        ...
        brain.submit_action(Action(name=ActionType.IDLE))
        t.join(timeout=2)
        assert not t.is_alive()
        assert result == [Action(name=ActionType.IDLE)]
    ...
    def test_on_turn_fires_before_blocking(self) -> None:
        ...
        def on_turn(
            ...
            brain.submit_action(Action(name=ActionType.SAY, params={"text": "hello"}))

        brain.set_on_turn(on_turn)  # type: ignore[arg-type]

        action = brain.choose_action(_make_creature(), _make_awareness(), [])
        assert action == Action(name=ActionType.SAY, params={"text": "hello"})
        assert turns == ["test"]
    ...
    def test_no_on_turn_still_works(self) -> None:
        ...
        # Pre-fill queue
        brain.submit_action(Action(name=ActionType.DODGE))
        action = brain.choose_action(_make_creature(), _make_awareness(), [])
        assert action == Action(name=ActionType.DODGE)

    def test_multiple_actions_queued(self) -> None:
        """Multiple actions can be queued and consumed in order."""
        brain = PlayerBrain()
        brain.submit_action(Action(name=ActionType.ATTACK, params={"target_id": "g1"}))
        brain.submit_action(Action(name=ActionType.DODGE))

        a1 = brain.choose_action(_make_creature(), _make_awareness(), [])
        a2 = brain.choose_action(_make_creature(), _make_awareness(), [])
        assert a1.name == ActionType.ATTACK
        assert a2.name == ActionType.DODGE
