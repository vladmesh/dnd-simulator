...
Each creature's turn is a multi-action loop orchestrated by `Round` (in `round.py`). A `TurnBudget` (actions, bonus_actions, movement_remaining, reaction) lives on `Creature.turn_budget` — persists between turns for reaction spending. Created fresh at the start of each creature's turn. The brain is called repeatedly: choose action → `ActionDispatcher` validates (budget, target, reach via `rules/validation.py`) → executes handler (`rules/handlers/`) → rebuilds awareness → repeat, until the brain returns `end_turn` or budget is exhausted. The dispatcher owns action, bonus action and reaction costs only. Movement is charged inside `rules/handlers/movement.py` by the distance actually moved, so `MOVE` and `MOVE_TO` are `CostType.FREE` at the dispatcher. `ActionProvider` (`rules/action_provider.py`) determines which actions are currently available to a creature. RuleBrain moves only by `MOVE_TO`: it rebuilds the battle map from `CombatAwareness` (bounds, `battle_map_walls`, `occupied_cells`), plans with `rules/movement.compute_reachable` to the cheapest free cell within its primary reach of the target (or the nearest other enemy with a path), walks as far along that path as its movement allows, Dashes only when a path exists but is longer than its movement, and Dodges when no enemy's attack cell is reachable; retreat and flee back-off move to the affordable cell farthest from the threats. `PlayerBrain` uses a queue + callback pattern for interactive I/O. `Round.stop()` (the last player disconnecting, load, eviction) interrupts mid-round: no further turn starts, a brain decision returning after the stop (including the END_TURN the session injects to wake a waiting player) is discarded, an action already dispatching completes, and the interrupted round advances no time and fires no round result. The combat cursor `resume_turn_index` + `resume_turn_started` (saved with the combat) makes the next round continue the interrupted turn with its persisted budget, without re-ticking conditions; peaceful turns have no cursor, so the rest of an interrupted peaceful round is skipped and the next round runs every active creature. A disconnected WebSocket listener is closed before the stop, so nothing is sent to a closed socket.
...
**Loot** — `InventoryHolder` Protocol (`core/loot.py`, anything with `inventory` + `gold`) with derived `is_lootable()` (dead creature or open container). The `take` action (`ActionType.TAKE`, `LootActionProvider`, handler in `rules/handlers/loot.py`) is take-all: one action transfers a holder's whole inventory + gold to the actor via the shared `rules/inventory.transfer_items` primitive (trade is refactored onto the same primitive; loot/trade/theft are separate access modes over it). In combat `take` costs the Action and the holder must be in the actor's or an adjacent cell (5 ft, diagonals count): `rules/loot.loot_reach` is the one rule, used by `take` validation and by `CombatAwareness.lootables` (per-holder `in_reach` + `reason_key`) for the player's loot panel. A creature that dies in a fight keeps its last cell in `BattleMap.corpses` (not an occupied cell, saved with the combat); a holder with no cell on the grid is not reachable in combat. Equipment costs follow 5e: weapon equip/unequip is free, shield is an Action, armor and accessories are peaceful-only. `CombatAwareness.blocked_actions` lists the equipment actions the creature has items for but cannot take now, with the validation code (`WRONG_MODE`, `INSUFFICIENT_BUDGET`) and localised message; the inventory panel disables those controls with that reason. Lair treasuries are persistent `Container` entities gated behind core death.
