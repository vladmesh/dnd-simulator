...
describe("SessionView — live observe feed", () => {
  it("renders live events read-only for a DM, with no action controls", async () => {
    ...
    // master screen has no action bar
  ...
  it("renders the same live feed for an admin while write tabs stay absent", async () => {
    const user = userEvent.setup()
    useGameStore.setState({ userId: "root", role: "admin" })
    renderView()

    await screen.findByText(/Day 1/)
    // observe mode: write tabs hidden
